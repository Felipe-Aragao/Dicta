...
function CopyLinkButton({ link }) {
  ...
  const handleCopy = (e) => {
    e.stopPropagation(); // evita abrir a linha da tabela
      ...
      <div className="modal-card">
        ...
        {!file ? (
          ...
          <div style={{
            ...
            <span style={{
              fontSize: "0.92rem", fontWeight: 600, color: "var(--green-700)",
        ...
        {loading && (
          <div style={{
            ...
            padding: "16px 0", color: "var(--text-3)", fontSize: "0.9rem",
          }}>
        ...
        <div className="modal-actions">
          <button className="btn btn-outline" onClick={onClose} disabled={loading} aria-label="Cancelar">
            Cancelar
          </button>
          <button className="btn btn-primary" disabled={!canCreate || loading} onClick={handleSubmit} aria-label="Criar atividade">
...
export function ProfessorScreen() {
  ...
  const handleCreate = (nome) => {
    // Gera um link fictício baseado no nome
              ...
              <tbody>
                {questionarios.map((q) => (
                  <tr
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") alert(`Abrindo: ${q.nome}`);
                    }}
                  >
                    <td>
