...
function NovaAtividadeModal({ onClose, onPreview }) {
  ...
  const handleNext = () => {
    if (!canNext) return;
    setLoading(true);
    // Simula leitura do PDF por 1.5s antes de mostrar preview
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    ...
    aria-label="Pré-visualizar prova antes de publicar">
...
export function ProfessorScreen({ username, onLogout }) {
  ...
  const [previewData, setPreviewData]     = useState(null); // { nome, professor, disciplina }
  ...
  return (
    ...
    {filtered.map((q) => (
      ...
      aria-label={`${q.nome}, ${q.professor}, ${q.disciplina}`}
      ...
      <span style={{ fontWeight: 600, color: "var(--text-1)", fontSize: "0.92rem" }}>{q.professor}</span>
      <span style={{ fontSize: "0.82rem", color: "var(--text-3)" }}>{q.disciplina}</span>
      </div>
      </td>
