...
QUESTION_START_PATTERNS = [
    re.compile(
        r"^\s*(?:quest(?:ão|ao)|exerc[ií]cio|pergunta|problema)\s*(?:n[ºo]\.?\s*)?(\d{1,3})\s*[:.)\-–—−]?\s*(.*)$",
        re.IGNORECASE,
    ),
...
FOOTER_STAMP_RE = re.compile(
    r"^(?=.*\b\d{2}/\d{2}/\d{4}\b)(?=.*\b\d{1,2}:\d{2}(?::\d{2})?\b)(?=.*[A-Z0-9_]{4,}).{1,120}$"
)
...
INLINE_LETTER_OPTION_RE = re.compile(
    r"(?<!\S)(?:\(([A-Ha-h])\)|([A-Ha-h])\s*[.):]|([A-H])\s*[\-–—−]|([a-h])\s*[\-–—−]\s+)\s+"
)
INLINE_CONTINUATION_OPTION_RE = re.compile(
    r"(?<!\S)(?:\(([A-Ha-h])\)|([A-Ha-h])\s*[.):]|([A-H])\s*[\-–—−]|([a-h])\s*[\-–—−]\s+)\s*"
)
LETTER_OPTION_RE = re.compile(
    r"^\s*(?:\(([A-Ha-h])\)|([A-Ha-h])\s*[.):]|([A-H])\s*[\-–—−]|([a-h])\s*[\-–—−]\s+)\s*(.*)$"
)
ROMAN_OPTION_RE = re.compile(
    r"^\s*(?:\((I|II|III|IV|V|VI|VII|VIII|IX|X)\)|((?:I|II|III|IV|V|VI|VII|VIII|IX|X))\s*[.)\-–—−:])\s*(.*)$",
    re.IGNORECASE,
)
PAGE_FOOTER_RE = re.compile(
    r"^(?=.*\b\d{1,3}\b)(?=.*(?:confidencial|aplica(?:ç|c)(?:ão|ao))).{1,120}$",
    re.IGNORECASE,
)
EMBEDDED_PAGE_FOOTER_RE = re.compile(
    r"\s+\d{1,3}\s+\S{6,50}.*?(?:confidencial|aplica(?:ç|c)(?:ão|ao)).*$",
    re.IGNORECASE,
)
EMBEDDED_ANSWER_SHEET_RE = re.compile(
    r"\s+(?:cart[aã]o|folha)\s+(?:de\s+)?respostas?.*$",
    re.IGNORECASE,
)
EMBEDDED_CODE_TAIL_RE = re.compile(
    r"\s+\S{4,24}\s+(?=[A-Za-z0-9_+/:-]{18,}={0,2}\b)(?=\S*(?:\d|[+=]))[A-Za-z0-9_+/:-]+={0,2}.*$"
)
EMBEDDED_SCRATCH_RE = re.compile(
    r"\s+(?:r\s*a\s*s\s*c\s*u\s*n\s*h\s*o|draft|scratch)\b.*$",
    re.IGNORECASE,
)
EMBEDDED_NUMERIC_QUESTION_RE = re.compile(r"\s+(\d{2,3}[.)]\s+\S.{11,})$")
WEAK_PROMPT_RE = re.compile(
    r"^(?:instru(?:ç|c)(?:ões|oes)|orienta(?:ç|c)(?:ões|oes))\b|^(?:nome|aluno|professor|turma|data|disciplina|curso)\b.{0,40}:\s*\S*$",
    re.IGNORECASE,
)
TRAILING_CONTENT_RE = re.compile(
    r"^(?:resposta|answer|gabarito|corre(?:ç|c)(?:ão|ao)|coment[aá]rio|feedback|pontua(?:ç|c)(?:ão|ao)|nota|gerado(?:\s+em)?|criado(?:\s+em)?|emitido(?:\s+em)?|assinado(?:\s+por)?|edital)\s*:",
    re.IGNORECASE,
)
METADATA_LABEL_RE = re.compile(
    r"^(?:nome|aluno|professor|turma|data|inscri(?:ç|c)(?:ão|ao)|cpf|rg|assinatura|cargo|sala|pr[eé]dio|carteira)\b.{0,40}:\s*\S*",
    re.IGNORECASE,
)
SUPPORT_TEXT_RE = re.compile(
    r"leia\s+(?:o|a|os|as)\s+(?:texto|poema|charge|tirinha|excerto|fragmento|not[ií]cia|cr[oô]nica|cartum|passagem)s?.*?"
    r"(?:quest(?:ões|oes)|n[uú]meros?)\s+(?:de\s+)?(\d{1,3})\s*(?:a|à|e|-|–|—)\s*(\d{1,3})",
    re.IGNORECASE,
)
SECTION_RE = re.compile(
    r"^(?:confidencial|instru(?:ç|c)(?:ões|oes)\s+gerais|orienta(?:ç|c)(?:ões|oes)\s+gerais)\b",
    re.IGNORECASE,
)
INSTRUCTION_START_RE = re.compile(
    r"^(?:instru(?:ç|c)(?:ões|oes)|orienta(?:ç|c)(?:ões|oes)|observa(?:ç|c)(?:ões|oes)).{0,40}(?:candidato|prova|gerais)?\s*:?\s*$",
    re.IGNORECASE,
)
INSTRUCTION_END_RE = re.compile(
    r"^(?:boa\s+prova|concurso\s+p[uú]blico|cargo|l[ií]ngua|matem[aá]tica|conhecimentos|direito|inform[aá]tica|atualidades)\b",
    re.IGNORECASE,
)
...
def _text_usefulness_score(text: str) -> int:
    ...
    score = len(text)
    question_markers = sum(1 for line in text.splitlines() if _question_start_match(line))
    score += question_markers * 500

    noisy_chars = sum(len(match.group(0)) for match in URL_RE.finditer(text))
    noisy_chars += sum(len(match.group(0)) for match in TOKEN_CODE_RE.finditer(text))
    if noisy_chars >= max(40, len(text) * 0.45):
        score -= noisy_chars * 3
...
def _line_is_mostly_noise(line: str, *patterns: re.Pattern) -> bool:
    ...
    if matched < max(18, len(line) * 0.55):
        return False

    remainder = line
    for pattern in patterns:
        remainder = pattern.sub(" ", remainder)
    meaningful_remainder = "".join(char for char in remainder if char.isalnum())
    return len(meaningful_remainder) <= max(8, len(line) * 0.18)
...
def _question_start_match(line: str) -> re.Match | None:
    for index, pattern in enumerate(QUESTION_START_PATTERNS):
        ...
        if index < 2 or not marker_text or len(marker_text) >= 8:
            return match
    return None
...
def _is_plain_numbered_question_match(line: str, match: re.Match | None) -> bool:
    if not match:
        return False
    if _explicit_question_start_match(line):
        return False
...
def _split_inline_letter_options(line: str) -> list[str]:
    ...
    if matches and matches[0].start() == 0:
        if not _has_sequential_inline_options(matches):
            return [line]
    else:
        continuation_matches = list(INLINE_CONTINUATION_OPTION_RE.finditer(line))
        if not _has_sequential_inline_options(continuation_matches):
            return [line]

        prefix = line[: continuation_matches[0].start()].strip()
        if not prefix or (not prefix.endswith((".", "?", "!", ":")) and len(prefix) < 16):
            return [line]

        split_lines = [prefix]
        for index, match in enumerate(continuation_matches):
            end = (
                continuation_matches[index + 1].start()
                if index + 1 < len(continuation_matches)
                else len(line)
            )
...
def _split_embedded_question_start(line: str) -> list[str]:
    ...
    if not before or not _question_start_match(after):
        return [line]
    return [before, after]
...
def _split_embedded_question_lines(lines: list[str]) -> list[str]:
    split_lines = []
    for line in lines:
        split_lines.extend(_split_embedded_question_start(line))
    return split_lines
...
def _looks_like_instruction_item(line: str) -> bool:
    ...
    if not _is_plain_numbered_question_match(line, match):
        return False
    ...
    return re.match(
        r"^(?:prova|justifique|responda|marque|assinale|preencha|utilize|use|n[aã]o|somente|apenas|cada\s+quest(?:ão|ao)|ser[aá]|boa\s+prova)\b",
        text,
        re.IGNORECASE,
    ) is not None
...
def _prepare_question_lines(text: str) -> list[str]:
    ...
    for raw_line in text.splitlines():
        ...
        if (
            lines
            and not starts_question
            and not starts_option
            and not starts_trailing
            and not starts_support
        ):
            previous = lines[-1]
...
def _trim_embedded_trailing_content(text: str) -> str:
    cut_positions = []
    for pattern in (
        EMBEDDED_PAGE_FOOTER_RE,
        EMBEDDED_ANSWER_SHEET_RE,
        EMBEDDED_CODE_TAIL_RE,
        EMBEDDED_SCRATCH_RE,
    ):
...
def _is_valid_prompt(prompt: str) -> bool:
    ...
    if len(prompt) < 12:
        return False
    if WEAK_PROMPT_RE.match(prompt):
        return False
...
def _looks_like_trailing_content(line: str) -> bool:
    ...
    if re.match(r"^\d{1,3}$", line):
        return True
    if _line_is_mostly_noise(line, URL_RE):
        return True
    if _line_is_mostly_noise(line, LONG_CODE_RE, TOKEN_CODE_RE):
        return True
    if FOOTER_STAMP_RE.match(line):
        return True
    if PAGE_FOOTER_RE.match(line):
        return True
    if _support_text_match(line):
        return True
    if _looks_like_section_header(line):
        return True
    if TRAILING_CONTENT_RE.match(line):
        return True
    if METADATA_LABEL_RE.match(line) and not _question_start_match(line):
        return True
    return False
...
def _looks_like_section_header(line: str) -> bool:
    line = line.strip()
    if len(line) > 90 or _question_start_match(line) or _looks_like_option(line):
        return False
    if line.endswith(("?", "!", ".", ";")):
        return False
...
def _looks_like_numbered_item_continuation(
    current_block: list[str], line: str, match: re.Match | None
) -> bool:
    if not current_block or not _is_plain_numbered_question_match(line, match):
        return False
    ...
    if (
        ...
        and len(current_marker) >= 3
        and len(candidate_marker) < len(current_marker)
    ):
        return False
    if _block_has_options(current_block):
        return (
            current_number > 10
            and candidate_number <= 10
            and candidate_number != current_number + 1
            and _is_dash_numbered_match(line)
        )
...
def _extract_options(block_lines: list[str]) -> tuple[list[str], list[str]]:
    ...
    for line in block_lines:
        expanded_lines.extend(_split_inline_letter_options(line))
    ...
    if letter_count == 0 and roman_count >= 2:
        use_roman = True

    for index, line in enumerate(expanded_lines):
        if index == 0:
            line = _strip_question_marker(line)
        ...
        if block_closed and not option_match:
            continue
        ...
        if (
            ...
            and current_option.strip()
            and not re.match(r"^\d{1,3}$", line.strip())
            and _looks_like_trailing_content(line)
        ):
            option_lines.append(_trim_embedded_trailing_content(current_option))
            current_option = None
            prompt_closed = True
    ...
    if len(clean_options) < 2:
        return prompt_lines + option_lines, []
...
def extract_questions_from_text(text: str, limit: int | None = None) -> list[ParsedQuestion]:
    ...
    blocks = []
    current_block = []
    prefer_explicit_question_markers = any(
        _explicit_question_start_match(line) for line in lines
    )
    ...
    for index, line in enumerate(lines):
        starts_question = (
            _explicit_question_start_match(line)
            if prefer_explicit_question_markers
            else _question_start_match(line)
        )
        if (
            first_numbered_question_index is not None
            and index < first_numbered_question_index
            and _is_plain_numbered_question_match(line, starts_question)
        ):
    ...
    for block in blocks:
        ...
        questions.append(
            ParsedQuestion(
                type="multiple" if options else "open",
                text=prompt,
                options=options,
            )
        )
    ...
    try:
        ...
        if reader.is_encrypted:
            ...
            if decrypt_result == 0:
                raise HTTPException(
                    status_code=422,
                    detail="PDF criptografado. Não foi possível extrair o texto.",
                )
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            if page_text:
                ...
                warnings.append(
                    f"Página {page_number} não contém texto pesquisável extraível."
                )
        ...
        if not text:
            warnings.append(
                "Nenhum texto pesquisável foi encontrado. O PDF pode ser escaneado ou conter apenas imagens."
            )
        ...
        raise HTTPException(
            status_code=422,
            detail="PDF inválido ou corrompido. Não foi possível extrair o texto.",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=500,
            detail="Falha ao extrair o texto do PDF.",
        ) from exc
...
async def receive_pdf(
    ...
    questions = extract_questions_from_text(extracted.text, limit=num_questions)
    warnings = list(extracted.warnings)
    if not questions:
        warnings.append("Nenhuma questão foi identificada no PDF.")
